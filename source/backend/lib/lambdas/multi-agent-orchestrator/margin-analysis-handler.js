/**
 * @fileoverview Margin Analysis Agent handler.
 * 
 * Invokes AgentCore Runtime Margin Analysis Agent deployed as a Lambda function.
 * Handles Step Functions task input reception with supervisor results,
 * AgentCore agent invocation, response processing, and DynamoDB updates.
 * Includes performance monitoring with CloudWatch metrics and duration tracking.
 * 
 * Requirements: FR-4, FR-6, FR-7, FR-8, NFR-1, NFR-2, NFR-5
 */
...
const AGENTCORE_ID = process.env.MARGIN_ANALYSIS_AGENTCORE_ID;
const AGENT_NAME = 'Margin Analysis Agent';
...
/**
 * Polls AgentCore Runtime agent for completion with message collection.
 * 
 * Polls every 2 seconds for up to 30 seconds to check agent completion status.
 * Collects messages during polling for real-time updates.
 * 
 * @param {string} agentCoreId - AgentCore Runtime agent ID
 * @param {string} sessionId - Session identifier
 * @param {number} timeout - Timeout in milliseconds (default 30000)
 * @returns {Promise<Object>} Agent result with analysis and messages
 * @throws {Error} If agent execution times out or fails
 */
// Removed pollAgentCompletion - AgentCore agents now use synchronous Lambda invocation

/**
 * Lambda handler for Margin Analysis Agent invocation via AgentCore Runtime.
 * 
 * Receives Step Functions task input with supervisor results, invokes AgentCore Runtime agent
 * (deployed as Lambda), processes response, updates DynamoDB, and returns results to Step Functions.
 * Includes performance monitoring with duration tracking and CloudWatch metrics.
 * 
 * Requirements: FR-4, FR-6, FR-7, FR-8, NFR-1, NFR-2, NFR-5
 * 
 * @param {Object} event - Step Functions task input with supervisor results
 ...
 * @param {Object} event.supervisorResults - Results from demand forecast and competitive analysis agents
 * @returns {Promise<Object>} Agent execution result with performance metrics
 * @throws {Error} If agent invocation fails
 */
exports.handler = async (event) => {
  const { sessionId, userId, product, supervisorResults } = event;
  
  // Create performance monitor for tracking execution time (Requirement NFR-1, NFR-5)
  const perfMonitor = createPerformanceMonitor(sessionId, 'MarginAnalysisAgent');
  
  console.log(`[Margin Analysis Handler] Invoking ${AGENT_NAME} for session ${sessionId}`);
  console.log(`[Margin Analysis Handler] Event:`, JSON.stringify(event, null, 2));
  ...
  try {
    ...
    // Include supervisor results in the input for margin analysis
    const response = await invokeAgentCoreAgent({
      ...
      input: { 
        ...
        userId, 
        product,
        supervisorResults: supervisorResults || {} // Include supervisor results for margin analysis
      },
    ...
    console.log(`[Margin Analysis Handler] Agent invocation completed`, {
    ...
    if (response.status === 'failed') {
      throw new Error(response.error || 'Agent execution failed without error message');
    }
    
    // Validate execution time against threshold (Requirement NFR-1)
    const timeValidation = validateAgentExecutionTime(response.duration || invokeDuration, AGENT_NAME);
    logInfo(sessionId, 'MarginAnalysisAgent', timeValidation.message, {
    ...
    await updateSessionStatus(sessionId, 'success', {
      ...
      marginAnalysis: result.analysis,
      dataSource: result.data_source,
      confidenceScore: result.confidence_score,
    ...
    console.log(`[Margin Analysis Handler] ${AGENT_NAME} completed successfully for session ${sessionId}`);
    ...
    console.error(`[Margin Analysis Handler] ${AGENT_NAME} failed:`, error.message);
    ...
    logError(sessionId, 'MarginAnalysisAgent', error, {
      agentCoreId: AGENTCORE_ID,
      agentName: AGENT_NAME,
    ...
    try {
      await updateSessionStatus(sessionId, 'error', {
        currentAgent: AGENT_NAME,
        agentStatus: 'failed',
        error: formatErrorForStorage(error, 'MarginAnalysisAgent'),
        endTime: new Date().toISOString(),
        elapsedTime: perfMonitor.getElapsedTime(),
        performanceMetrics: perfMonitor.getSummary()
      });
      ...
      logError(sessionId, 'MarginAnalysisAgent', updateError, {
